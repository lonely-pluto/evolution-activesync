...
:||{
match=0
for account in `gsettings get org.gnome.activesyncd accounts`
do
	account=${account%[\']*}
	account=${account#*[\']}
...
if [ "$match" -eq "0" ]
then
	gsettings set org.gnome.activesyncd accounts "`gsettings get org.gnome.activesyncd accounts | sed s/]//`, '$EMAILADDR']"
fi

gsettings set org.gnome.activesyncd.account:/org/gnome/activesyncd/account/$EMAILADDR/ username "$ASUSERNAME"
gsettings set org.gnome.activesyncd.account:/org/gnome/activesyncd/account/$EMAILADDR/ serveruri "$SERVERURL"
}
